...
public class FilterSecurityInterceptor extends AbstractSecurityInterceptor implements Filter {
    ...
    public void invoke(FilterInvocation fi) throws IOException, ServletException {
        if ((fi.getRequest() != null) && (fi.getRequest().getAttribute(FILTER_APPLIED) != null)
            ...
            //如果用户已经登录,访问登录页面时，则跳转到用户的默认主页面
            if (fi.getRequest() != null && fi.getRequestUrl().equals("/login.jsp")) {
                UsernamePasswordAuthenticationToken token = (UsernamePasswordAuthenticationToken)fi.getRequest().getUserPrincipal();
                if (token != null) {
                    MemUser user = (MemUser) token.getPrincipal();
                    if (user != null) {
                        redirectStrategy.sendRedirect(fi.getHttpRequest(), fi.getHttpResponse(), user.getHomepage());
                        return;
                    }
                }
            }
