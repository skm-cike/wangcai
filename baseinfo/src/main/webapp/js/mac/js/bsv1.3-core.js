	...
	return me ={
		 ...
		 bindTheme:function(){
			 ...
			 $("a",themsSetting).click(function(){
					...
					var h = $(window).height();
					var w = $(window).width();
					$("#zoomWallpaper").attr("src",_basepath+"img/background/theme_"+src+".jpg").width(w).height(h);
...
Navbar =function(me){
	...
	return me = {
		...
		bindSwitchDesktopAnimate:function(t,c){//切换动画事件 t 目标桌面  c当前桌面
			...
			idesk.offset({left:left * -1,top:idesk.offset().top});
			cdesk.stop().animate({
				left: left
...
//初始化分页箭头
Arrows = function(){
	var arrows_l = "<div id='arrows_l' class='arrows'><img src='"+_basepath+"js/mac/images/arrows_l_3.png' /></div>";
	var arrows_r = "<div id='arrows_r' class='arrows'><img src='"+_basepath+"js/mac/images/arrows_r_1.png' /></div>";
	return arrows = {
		init:function(){
			arrows.create();
			arrows.setPosition();
		...
		changeStyle:function(t,c){
			...
			if(t>1 && t<DATA.menu.length){
				$("#arrows_l img").attr("src",_basepath+"js/mac/images/arrows_l_1.png");
				$("#arrows_r img").attr("src",_basepath+"js/mac/images/arrows_r_1.png");
			}
			if(t==1){
				$("#arrows_l img").attr("src",_basepath+"js/mac/images/arrows_l_3.png");
				$("#arrows_r img").attr("src",_basepath+"js/mac/images/arrows_r_1.png");
			}
			if(t==DATA.menu.length){
				$("#arrows_l img").attr("src",_basepath+"js/mac/images/arrows_l_1.png");
				$("#arrows_r img").attr("src",_basepath+"js/mac/images/arrows_r_3.png");
			}
		}
	}
}();
	...
	var _folder = "<div id=\"folder\"><a href=\"#\">"+
   		"<div id=\"folder_content\">"+
			"<div id=\"min_icon_folder\">"+
            	"<img width=\"32\" height=\"32\" border=\"0\" src=\""+_basepath+"js/mac/icon/min/folder_o.png\" />"+
        	"</div>"+
	...
	return file = {
		...
		bindStyle:function(){
			$("#folder").mouseenter(function(){
				$("#folder").css({"background-image": "url(" + _basepath + "js/mac/images/bg_task_group_t_over.png)"});
			});
			$("#folder").mouseleave(function(){
				$("#folder").css({"background-image": "url(" + _basepath + "js/mac/images/bg_task_group_t_msg.png)"});
			});
	...
	return me = {
		...
		bindStyle:function(){
			$(".sonfile").mouseenter(function(){
				$(this).css({"background-image": "url(" + _basepath + "js/mac/images/bg_task_group_t_over.png)"});
			});
			$(".sonfile").mouseleave(function(){
				$(this).css({"background-image": "url(" + _basepath + "js/mac/images/bg_task_group_t.png)"});
			});
		},
		bindEvent:function(id){
			$(".sonMenuList").bind("mouseenter",function(){
				$(this).css({"background-image": "url(" + _basepath + "js/mac/images/appbutton_mouseover_bg3.png)"});
			});
				...
				var sonfile = "<a id=\"sonfile_a"+id+"\" href=\"#\">"+
					"<div id=\"sonfile_"+id+"\" class=\"sonfile\">"+
						"<div class=\"min_icon_sonfolder\">"+
							"<img width=\"32\" height=\"32\" border=\"0\" src='"+icon+"' />"+
						"</div>"+
					...
					for(var i=0;i<jsonSonMenu.length;i++ ){
						...
						sonlist += "<div id=\""+ jsonSonMenu[i].appid+"\" "+ properties +" class=\"sonMenuList\" align=\"center\">"+
							"<a href=\"#\"><div class=\"sonMenuListIocn\">"+
								"<img width=\"64\" height=\"64\" src='"+jsonSonMenu[i].icon+"' />"+
								"</div>"+
...
mouseStyle = function(){//箭头样式
	$("#arrows_l").mouseenter(function (){
		if(thisPage==1){
			...
			$("#arrows_l img").attr("src",_basepath+"js/mac/images/arrows_l_2.png");
		}
	});
	$("#arrows_r").mouseenter(function (){
		if(thisPage==DATA.menu.length){
			...
			$("#arrows_r img").attr("src",_basepath+"js/mac/images/arrows_r_2.png");
		}
	});
